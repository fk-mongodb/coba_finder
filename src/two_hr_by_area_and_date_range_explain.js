...
async function ops(client) {
  ...
  const customPromise = new Promise(async (resolve, reject) => {
    ...
    const all = "allPlansExecution";
    const resExplain = await forecastColl.aggregate(pipeline).explain(planner);
    console.log(
      resExplain.stages[0]["$cursor"].queryPlanner.winningPlan.inputStage
    ); 
    resolve();
  });
